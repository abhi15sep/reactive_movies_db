          ...
          {tv.networks.map((n, i) => {
            return (
              <div className="s12" key={i} >                
                {n.logo_path ? <img className="responsive-img" src={"https://image.tmdb.org/t/p/h30" + n.logo_path} alt={n.name} /> : <button className="btn grey">{n.name}</button>}
              </div>
            );
          })
          }
        ...
        <div className="container">
          <div className="row ">
            ...
            <div className="col s12 m12 l4 teal lighten-5 ">
              <h5>Facts</h5>
              <div>                          
               <a href={tv.homepage} className="btn-flat" target="_blank" rel="noopener noreferrer">
                <i className="material-icons left">insert_link</i></a>
